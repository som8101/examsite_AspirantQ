...
export default async function AdminAttemptDetailsPage(props: { params: Promise<{ id: string, attemptId: string }> }) {
  ...
  const { data: attempt } = await supabase
    .from('exam_attempts')
    .select('*, exams(*), profiles!student_id(full_name, login_id)')
  ...
  if (attempt.status === 'in_progress') {
    return (
      <div className="max-w-3xl mx-auto p-8 text-center bg-white rounded-lg shadow-sm">
        ...
        <h2 className="text-xl font-bold mb-2">Exam In Progress</h2>
        <p className="text-muted-foreground mb-6">The student is currently taking this exam. Results will be available once they submit.</p>
        <Link href={`/admin/exams/${params.id}/results`}>
  ...
  const { data: answers } = await supabase
    ...
    .order('question_number', { referencedTable: 'questions', ascending: true });
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
       <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">{studentName}'s Results</h1>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-4">
           ...
           {(!answers || answers.length === 0) && (
             <div className="text-center p-12 bg-slate-50 rounded-lg border">
               <p className="text-muted-foreground">No answers recorded for this attempt.</p>
             </div>
           )}
        </div>
      </div>
