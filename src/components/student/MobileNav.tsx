...
import { Menu, X, BookOpen, LayoutDashboard, Award, LogOut } from 'lucide-react';
...
export function MobileNav({ signoutAction }: { signoutAction: () => void }) {
  ...
  const links = [
    ...
    { href: '/student/dashboard', icon: Award, label: 'My Results' },
  ];
          ...
          <div className="flex flex-col flex-1 p-6 space-y-4">
            <nav className="flex flex-col space-y-2">
              {links.map((link, i) => {
                ...
                const isActive = pathname === link.href && (i === 0 ? true : false); // Both point to dashboard currently
                return (
                  <Link
                    key={i}
                    href={link.href}
                    ...
                    className={`flex items-center gap-4 px-4 py-3 rounded-xl text-lg font-medium transition-colors ${
                      isActive && i===0 ? 'bg-primary/10 text-primary' : 'hover:bg-card/50 text-foreground'
                    }`}
                  >
